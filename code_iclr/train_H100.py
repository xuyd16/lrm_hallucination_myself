...
# 导入模型和数据加载器
from model_H100 import (DualDomainTransformerMEM, ModelConfig, 
...
from load_H100 import EEGBrainRegionDataset, create_data_loader
...
if __name__ == "__main__":
    ...
    from load_H100 import custom_collate_fn
    ...
    train_loader = DataLoader(
        ...
        sampler=None, # 明确地不使用 sampler
        shuffle=True, # 【关键】将 shuffle 控制权交给 DataLoader
        num_workers=num_workers,
        pin_memory=True,
        collate_fn=custom_collate_fn,
        persistent_workers=True if num_workers > 0 else False,  # 保持worker进程活跃，减少创建开销
        prefetch_factor=4 if num_workers > 0 else 2  # 预取批次数量
    )
